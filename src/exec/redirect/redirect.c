...
void	redirect_open(t_cmd_token *cmd_token)
{
	while (cmd_token)
	{
		if (cmd_token->type == REDIRECT_OUT)
			redirect_out_open((t_redirect_param *) cmd_token->body);
		if (cmd_token->type == REDIRECT_IN)
			redirect_in_open((t_redirect_param *) cmd_token->body);
		if (cmd_token->type == APPEND_OUT)
			append_out_open((t_redirect_param *) cmd_token->body);
		if (cmd_token->type == HERE_DOC)
			here_doc_open((t_redirect_param *) cmd_token->body);
